...
class npc:
    ...
    # Given a type I error upper bound alpha and a violation upper bound beta,
    # npc calculates the Neyman-Pearson Classifier which controls the type I error
    # under alpha with probability at least 1-beta.
    # returns a result containing the fits with extra information
    def npc(self, x, y, method, alpha = None, beta = None, split = None, split_ratio = None, n_cores = None, band = None, rand_seed = None):
        ...
        if split_ratio == 'adaptive':
            ret = self.find_optim_split(x, y, method, alpha, beta, split, 10, band, rand_seed)
            split_ratio = ret[0]
            errors = ret[2]
        else:
            errors = 0
        ...
        if split == 0:
            ...
            fits = self.npc_split(x, y, p, alpha, beta, indices0, indices0, indices1, indices1, method, n_cores)
        else:
            # with split
            ...
            for i in range(split):
                ...
                if (band == True):
                    ...
                    fits.append(
                            self.npc_split(x, y, p, alpha, beta, indices0train, indices0test,
    ...
    def find_optim_split(self, x, y, method, alpha, beta, split, n_folds, band, rand_seed):
        # TODO
    ...
    # NPC split
    # returns a fit
    def npc_split(self, x, y, p, alpha, beta, indices0train, indices0test, indices1train, indices1test, method, n_cores):
        ...
        obj = self.npc_core(y_test, y_decision_values, alpha, beta, n_cores)
    ...
    def npc_core(self, y_test, y_decision_values, alpha, beta, n_cores):
        ...
        obj = self.find_order(test_list0, test_list1, beta, n_cores)
    ...
    # at least 1-beta
    def find_order(self, score0, score1 = None, beta = None, n_cores = None):
        ...
        def alpha_helper(s):
            #alpha_u = v_list[min(which(pbinom(n0 - ru0[s], len0, v_list) <= beta))]
            ...
            if r_upper0[s] == len0:
                ...
                #alpha_l = v_list[max(which(pbinom(n0 - rl0[i], n0, v_list) >= 1 - beta))]
            ...
            if r_upper1[s] >= len1:
                #beta_l = v_list[max(which(pbinom(rl1[i]-1, n1, v_list) >= 1 - beta))]
                ...
                #beta_u = v_list[min(which(pbinom(ru1[i]-1, n1, v_list) <= beta))]
